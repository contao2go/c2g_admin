                ...
                if(is_dir(TL_ROOT.'/vhosts/'.$hosts))
                {
                    ...
                    if(!$GLOBALS['TL_CONFIG']['c2g_disablesnapshoting'])
                    {
                        $arrOutput['buttons'][]=sprintf('
												...
												</a>', $pathBackups, htmlspecialchars($hosts), $GLOBALS['TL_LANG']['tl_content']['c2g_createbackup']);

                        $arrOutput['buttons'][]=sprintf('
												<a href="%s?dir=%s&amp;act=copy" title="%s">
													<img src="system/themes/default/images/copy.gif" alt="&nbsp;" />
												</a>', $this->pathMySelf, htmlspecialchars($hosts), $GLOBALS['TL_LANG']['tl_content']['c2g_copyvhost']);
                        ...
                        $arrOutput['buttons'][]=sprintf('
											...
											</a>', $this->pathMySelf, htmlspecialchars($hosts), $GLOBALS['TL_LANG']['tl_content']['c2g_deletepackage'], sprintf($GLOBALS['TL_LANG']['tl_content']['c2g_deletevhosts_confirm'], $hosts), $GLOBALS['TL_LANG']['tl_content']['c2g_pleasewait']);
                    }
        ...
        if(($this->Input->post("FORM_SUBMIT")=='c2g_copyvhost')&&($sourceDir!=$destDir))
        {
            ...
            foreach($arrSourceFiles as $file)
            {
                if(is_file(TL_ROOT.'/'.$file))
                {
                    ...
                    {
                        $strData=$this->c2g_functions->rewriteLocalconfig($strData, $GLOBALS['TL_CONFIG']['dbHost'], $GLOBALS['TL_CONFIG']['dbPort'], $newDB, $GLOBALS['TL_CONFIG']['dbUser'], $GLOBALS['TL_CONFIG']['dbPass'], '/vhosts/'.$destDir);
            ...
            $arrConfigReturn=$this->c2g_functions->loadVHostConfig(TL_ROOT.'/vhosts/'.$sourceDir, true);
